  ...
  try {
    ...
    if (user) {
      return res.status(400).json({
        success: false,
        msg: `An Account with username:  ${newUser.username} already exists`
      });
    }
    ...
    if (user) {
      return res.status(400).json({
        success: false,
        msg: `An Account with email:  ${newUser.email} already exists`
      });
    }
    ...
    User.addUser(newUser, (err, user) => {
      if (err) res.status(500).json({
        ...
        token.save(err => {
          if (err) res.status(500).json({
            success: false,
            msg: `Encountered and Unknown error: ${err}`
          });
          // Send the email
